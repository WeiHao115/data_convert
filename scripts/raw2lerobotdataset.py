...
class Config:
    # 路径配置
    DATASET_ROOT_DIR = "/home/k202/0421"
    ...
    TARGET_FPS = 30
    STATE_DIM = 8
    RESIZE_GOPRO = (224, 224)   
    RESIZE_TACTILE = (224, 224)
    RESIZE_REALSENSE = (640, 480)
...
def process_kinematics(episode_dir):
    ...
    if os.path.exists(path_txt):
        cols = ['timestamp', 'ee_x', 'ee_y', 'ee_z', 'ee_qx', 'ee_qy', 'ee_qz', 'ee_qw', 'gripper','Fx','Fy','Fz','Mx','My','Mz']
        df = pd.read_csv(path_txt, sep='\s+', comment='#', names=cols)
        return df['timestamp'].values, df.iloc[:, 1:].values
    elif os.path.exists(path_csv):
        df = pd.read_csv(path_csv)
        cols = ['ee_x', 'ee_y', 'ee_z', 'ee_qx', 'ee_qy', 'ee_qz', 'ee_qw', 'gripper','Fx','Fy','Fz','Mx','My','Mz']
    ...
    #处理六维力
    #force_data = np.loadtxt(force_files[0])
    # force_timestamps = force_data[: , 0]
    # raw_force = force_data[: , 1:]
    ...
    # 将六维力对齐至主体时间戳
    # force_aligned = np.zeros((len(timestamps), 1))
    # for i in range(len(timestamps)):
    #     time_diff = np.abs(force_timestamps - timestamps[i])
    #     closest_idx = np.argmin(time_diff)
    #     force_aligned[i, :] = raw_force[closest_idx]
...
def get_feature_config():
    h_gop, w_gop = Config.RESIZE_GOPRO[1], Config.RESIZE_GOPRO[0]
    # h_tac, w_tac = Config.RESIZE_TACTILE[1], Config.RESIZE_TACTILE[0]
    h_rs, w_rs = Config.RESIZE_REALSENSE[1], Config.RESIZE_REALSENSE[0]
    return {
        "observation.state": {
            "dtype": "float32",
            "shape": (Config.STATE_DIM,),
            "names": ["x", "y", "z", "qx", "qy", "qz", "qw", "gripper"],
            #"names": ["x", "y", "z", "qx", "qy", "qz", "qw", "gripper",'Fx','Fy','Fz','Mx','My','Mz'],
        },
        "action": {
            "dtype": "float32",
            "shape": (Config.STATE_DIM,), 
...
def main():
    ...
    with VideoEncodingManager(dataset):
            for ep_dir in episode_dirs:
                ...
                # 3. 解析触觉图像路径
                # dir_tl = os.path.join(ep_dir, "tactile_left")
                # dir_tr = os.path.join(ep_dir, "tactile_right")
                # ts_tac_left, files_tac_left = get_image_files(dir_tl)
                # ts_tac_right, files_tac_right = get_image_files(dir_tr)

                # 4. 解析REALSENSE图像路径
                dir_rs = os.path.join(ep_dir, "realsense")
  
                ts_rs, files_rs = get_image_files(dir_rs)
                ...
                for i in tqdm(range(num_samples - 1), desc=ep_name, leave=False):
                    ...
                    # path_tl = files_tac_left[find_nearest_index(ts_tac_left, curr_target_time)] if len(ts_tac_left) > 0 else None
                    # path_tr = files_tac_right[find_nearest_index(ts_tac_right, curr_target_time)] if len(ts_tac_right) > 0 else None
                    path_rs = files_rs[find_nearest_index(ts_rs, curr_target_time)] if len(ts_rs) > 0 else None

                    img_wr = safe_load_and_resize_image(path_wr, Config.RESIZE_GOPRO)
                    # img_tl = safe_load_and_resize_image(path_tl, Config.RESIZE_TACTILE)
                    # img_tr = safe_load_and_resize_image(path_tr, Config.RESIZE_TACTILE)
                    img_rs = safe_load_and_resize_image(path_rs, Config.RESIZE_REALSENSE)
                    lerobot_frame = {
                        "observation.state": curr_state.astype(np.float32),
                        "action": action,
                        "observation.images.gopro": img_wr,
                        "observation.images.realsense": img_rs,                        
